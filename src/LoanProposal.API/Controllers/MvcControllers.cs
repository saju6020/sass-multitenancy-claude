...
public class DashboardController : Controller
{
    ...
    public DashboardController(PlatformDbContext platformDb, TenantDbContextFactory tenantDbFactory)
    {
    ...
    public async Task<IActionResult> Index(CancellationToken ct) =>
        View(new DashboardViewModel { Tenants = await TenantSummariesAsync(_platformDb, _tenantDbFactory, User, ct) });

    internal static async Task<IReadOnlyList<TenantSummaryViewModel>> TenantSummariesAsync(PlatformDbContext platformDb, TenantDbContextFactory tenantDbFactory, ClaimsPrincipal user, CancellationToken ct)
    {
        var tenantsQuery = platformDb.Tenants.AsNoTracking().AsQueryable();
        if (!user.IsInRole("PlatformAdmin") && MvcCrudHelpers.CurrentTenantId(user) is Guid tenantId)
            tenantsQuery = tenantsQuery.Where(t => t.Id == tenantId);

        var tenants = await tenantsQuery.OrderBy(t => t.Name).ToListAsync(ct);
...
public class TenantsController : Controller
{
    ...
    public TenantsController(PlatformDbContext platformDb, TenantDbContextFactory tenantDbFactory)
    {
        _platformDb = platformDb;
        _tenantDbFactory = tenantDbFactory;
    }
    ...
    [HttpGet("")]
    public async Task<IActionResult> Index(CancellationToken ct) =>
        View(new TenantListViewModel { Tenants = await DashboardController.TenantSummariesAsync(_platformDb, _tenantDbFactory, User, ct) });
    ...
    public async Task<IActionResult> Create(CreateTenantForm form, CancellationToken ct)
    {
        ...
        var tenant = Tenant.Create(form.Name.Trim(), slug, form.Currency.Trim(), form.Timezone.Trim());
        tenant.ConfigureDatabase(_tenantDbFactory.BuildDatabaseName(slug), _tenantDbFactory.BuildConnectionString(slug));
        ...
        TempData["Success"] = "Tenant created with a dedicated database.";
        return RedirectToAction(nameof(Index));
    }
    ...
    public async Task<IActionResult> Edit(Guid id, TenantEditForm form, CancellationToken ct)
    {
        ...
        if (string.IsNullOrWhiteSpace(tenant.DatabaseConnectionString))
            tenant.ConfigureDatabase(_tenantDbFactory.BuildDatabaseName(tenant.Slug), _tenantDbFactory.BuildConnectionString(tenant.Slug));
    ...
    public async Task<IActionResult> Delete(Guid id, CancellationToken ct)
    {
        ...
        await _platformDb.SaveChangesAsync(ct);
        TempData["Success"] = "Tenant deleted from platform registry. Tenant database was left intact.";
        return RedirectToAction(nameof(Index));
    }
...
public class ConfigurationController : Controller
{
    ...
    public ConfigurationController(PlatformDbContext platformDb, TenantDbContextFactory tenantDbFactory)
    {
        _platformDb = platformDb;
        _tenantDbFactory = tenantDbFactory;
    }
    ...
    {
        var tenants = await GetTenantOptionsAsync(_platformDb, User, ct);
    ...
    private async Task<AppDbContext?> OpenTenantDbAsync(Guid tenantId, CancellationToken ct)
    {
        ...
        var tenant = await _platformDb.Tenants.AsNoTracking().FirstOrDefaultAsync(t => t.Id == tenantId, ct);
        return tenant is null ? null : _tenantDbFactory.CreateDbContext(tenant);
    }
}
...
public class ApplicationsController : Controller
{
    ...
    public ApplicationsController(PlatformDbContext platformDb, TenantDbContextFactory tenantDbFactory)
    {
        _platformDb = platformDb;
        _tenantDbFactory = tenantDbFactory;
    }
    ...
    public async Task<IActionResult> Advance(Guid id, Guid tenantId, string action, CancellationToken ct)
    {
        ...
        switch (action)
        {
            case "send_review":
                ...
                break;
            case "approve":
                if (!User.IsInRole("Approver") && !User.IsInRole("TenantAdmin") && !User.IsInRole("PlatformAdmin")) return Forbid();
...
internal static class MvcCrudHelpers
{
    ...
    public static bool CanAccessTenant(ClaimsPrincipal user, Guid tenantId) =>
        user.IsInRole("PlatformAdmin") || CurrentTenantId(user) == tenantId;

    public static async Task<IReadOnlyList<TenantOption>> GetTenantOptionsAsync(PlatformDbContext db, ClaimsPrincipal user, CancellationToken ct)
    {
        var query = db.Tenants.AsNoTracking().AsQueryable();
        if (!user.IsInRole("PlatformAdmin") && CurrentTenantId(user) is Guid tenantId)
